...
function OcHero() {
  return (
    <section className="relative overflow-hidden bg-linear-to-br from-brand-pink-pale via-white to-brand-purple-pale py-12 lg:py-20">
      <Container>
        <div className="mx-auto max-w-lg text-center">
          ...
          <h1 className="font-heading text-[28px] leading-[1.35] font-black text-brand-ink sm:text-[34px] lg:text-[42px]">
            家にいながら、
            <br />
            オンラインで相談。
            <br />
          ...
          <p className="mt-4 text-sm leading-relaxed text-brand-ink-soft lg:text-[15px]">
            AGA・美容内服・ダイエット・EDなど、
...
const benefits = [
  {
    icon: HomeIcon,
    title: "来院不要",
    desc: "自宅や職場など、好きな場所からスマホで受診できます。クリニックへ足を運ぶ必要はありません。",
  },
  {
    icon: DoctorIcon,
    title: "医師によるオンライン診察",
  ...
  },
  {
    icon: BoxIcon,
    title: "お薬はご自宅へ配送",
    desc: "処方が決まったお薬をご自宅へお届けします。受け取りに行く手間がありません。",
  },
  {
    icon: HeadsetIcon,
    title: "LINEから気軽に相談スタート",
    desc: "難しい手続きは不要。LINEに登録するだけで、まずは対象かどうか確認できます。",
  },
];
    ...
    <section className="bg-white py-10 lg:py-16">
      <Container>
        <SectionHeading
          ...
          description="通院の手間を減らして、気になる悩みをいつでもご相談いただけます。"
        />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {benefits.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="flex flex-col items-center rounded-2xl border border-brand-purple-light bg-brand-purple-pale/40 p-6 text-center shadow-sm"
            >
              <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm">
                <Icon className="h-6 w-6 text-brand-purple" />
              </div>
              <h3 className="text-[14px] font-bold text-brand-ink">{title}</h3>
              <p className="mt-2 text-[12.5px] leading-relaxed text-brand-ink-soft">{desc}</p>
            </div>
          ))}
        </div>
      </Container>
    ...
    <section className="bg-linear-to-br from-brand-pink-pale via-white to-brand-purple-pale py-10 lg:py-16">
      <Container>
        <div className="mx-auto max-w-2xl">
          ...
          />
          <div className="rounded-3xl border border-brand-pink-light bg-white p-6 shadow-md lg:p-10">
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand-pink-pale">
                <CheckIcon className="h-6 w-6 text-brand-pink-deep" />
              </div>
              <div>
            ...
            <div className="mt-6 rounded-xl bg-brand-pink-pale/60 px-4 py-3 text-[11.5px] leading-relaxed text-brand-ink-soft">
    ...
    <section className="bg-white py-10 lg:py-16">
      <Container>
        <SectionHeading
          eyebrow="CATEGORY"
          title="相談できるカテゴリ"
          description="医師の診察が必要なお悩みを、スマホからオンラインでご相談いただけます。"
        />
